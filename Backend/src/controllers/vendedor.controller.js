import {pool} from '../db/dbConfig.js'

export const getVendedor = async(req, res) => {
    try{
        const result = await pool.query('SELECT * FROM vendedor');
        res.json(result.rows);
    } catch (err) {
        console.error(err);
        res.status(500).jason({ error: 'Internal Server Error' });
    }
}

export const createVendedor = async(req, res) => {
    try {
        let { nombre, apellido1, apellido2 } = req.body;
        console.log({ nombre, apellido1, apellido2 });
        const result = await pool.query('INSERT INTO vendedor (nombre,apellido1,apellido2) VALUES ($1, $2, $3) RETURNING *', [nombre, apellido1, apellido2]);
        res.json(result.rows[0]);
    } catch (error) {
        console.error('Error inserting vendedor:', error);
        res.status(500).json({ error: error.message });
    }
}

export const deleteVendedor = async(req, res) => {
    try {
        const { id } = req.params;
        await pool.query('DELETE FROM vendedor WHERE id = $1', [id]);
        res.json({ message: 'Vendedor eliminado exitosamente' });
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: 'Internal Server Error' });
    }
}




